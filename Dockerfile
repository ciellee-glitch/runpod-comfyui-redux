...
RUN python3 -c "import facexlib; import os; print(os.path.dirname(facexlib.__file__) + '/weights/')" && \
    WEIGHTS_DIR=$(python3 -c "import facexlib, os; print(os.path.dirname(facexlib.__file__) + '/weights/')") && \
    curl -fsSL "https://github.com/xinntao/facexlib/releases/download/v0.1.0/detection_Resnet50_Final.pth" -o "${WEIGHTS_DIR}detection_Resnet50_Final.pth" && \
    curl -fsSL "https://github.com/xinntao/facexlib/releases/download/v0.2.2/parsing_parsenet.pth" -o "${WEIGHTS_DIR}parsing_parsenet.pth" && \
    curl -fsSL "https://github.com/xinntao/facexlib/releases/download/v0.2.0/parsing_bisenet.pth" -o "${WEIGHTS_DIR}parsing_bisenet.pth"
